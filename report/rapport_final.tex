...
<<<<<<< Updated upstream
		...
		\hline
		\textbf{Total} & --                            & --               & \textbf{10.92 h-coeurs} \\
=======
		\textbf{Run}   & \textbf{Configuration}         & \textbf{Durée}    & \textbf{Coût total} \\
		\hline
		RUN 1          & 1 noeud, 1 task, 1 CPU         & 3 $\times$ 5 min  & 0.25 h              \\
		RUN 2          & 1 noeud, 1 task, 1 CPU         & 3 $\times$ 20 min & 1.00 h              \\
		RUN 3          & 1 noeud, 4 tasks, 1 CPU        & 3 $\times$ 25 min & 5.00 h              \\
		RUN 4          & 1 noeud, 4 tasks, 1 CPU        & 4 $\times$ 10 min & 2.67 h              \\
		RUN 5          & 2 noeuds, 2 tasks/noeud, 1 CPU & 1 $\times$ 30 min & 2.00 h              \\
		\hline
		\textbf{Total} & --                             & --                & \textbf{10.92 h}    \\
>>>>>>> Stashed changes
		\hline
	\end{tabular}
...
Le suivi est effectué avec \texttt{sreport user TopUsage} (fenêtre temporelle explicite), afin de comparer la consommation réelle au plan prévisionnel et d'ajuster les campagnes avant le run multi-noeud final.
