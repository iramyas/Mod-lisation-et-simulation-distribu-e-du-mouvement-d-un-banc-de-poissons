...
int main() {
    ...
    applyText.setPosition({applyX + (applyBtn.getSize().x - applyText.getLocalBounds().getSize().x) / 2.f - 2.f,
                           applyY + (applyBtn.getSize().y - applyText.getLocalBounds().getSize().y) / 2.f - 5.f});
    ...
    while (window.isOpen()) {
        while (auto eventOpt = window.pollEvent()) {
            ...
            if (event.is<sf::Event::KeyPressed>()) {
                ...
                if (keyEvent && keyEvent->code == sf::Keyboard::Escape) {
                    window.close();
                }
            }
