...
void MPIManager::haloExchange(const std::vector<Boid>& localBoids,
    ...
    for (int dx = -1; dx <= 1; ++dx) {
        for (int dy = -1; dy <= 1; ++dy) {
            ...
            // Sélectionner les boids à envoyer (ceux près du bord)
...
void MPIManager::gatherBoids(const std::vector<Boid>& localBoids, std::vector<Boid>& globalBoids) {
    ...
    if (rank == 0) {
        for (int i = 0; i < size; ++i) {
            ...
            recvCounts[i] *= sizeof(Boid);  // Convertir en bytes
...
void MPIManager::scatterBoids(const std::vector<Boid>& globalBoids,
    ...
    // Rank 0 calcule la taille à distribuer à chaque processus
    if (rank == 0) {
        localSize = globalBoids.size() / size;
    }
    
    // Broadcast la taille à tous les processus
    ...
    // Broadcast les données à tous les processus
